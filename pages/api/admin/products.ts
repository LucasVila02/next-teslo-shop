...
const updateProduct=  async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    if(images.length < 2) {
        return res.status(400).json({ message: 'El necesario al menos 2 imagenes'})
    }

    //TODO: posiblemente tendremos un localhost:3000
    ...
    try {
        ...
        if(!product) {
            await db.disconnect();
            return res.status(400).json({ message: 'No existe un producto con es ID'})
        }
